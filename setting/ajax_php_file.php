...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_dir_file)) {
			$query = "INSERT INTO masjid_pekan_slider (`url`, `paparkan`) VALUES ('$target_file', '0')";
			if(mysqli_query($dbc,$query)){
				$sql = "SELECT `slider_id` FROM " . database_prefix ."_slider ORDER BY `slider_id` DESC LIMIT 1";
				$result = mysqli_query($dbc, $sql);
				$row = mysqli_fetch_assoc($result);
				$giliran = $row['slider_id'];
				$sql = "UPDATE " . database_prefix ."n_slider SET `giliran`='$giliran' WHERE `slider_id`='$giliran'";
				$result = mysqli_query($dbc, $sql);
				echo 'berjaya';
			}
    } else {
